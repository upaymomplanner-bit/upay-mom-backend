...
class MeetingDatabaseService:
    ...
    async def save_meeting(
        self, meeting_result: MeetingExtractionResult, host_id: Optional[UUID] = None
    ) -> UUID:
        """
    ...
    async def _save_goal(self, task_group: TaskGroup, year: int) -> UUID:
        ...
        if not result.data:
            raise Exception(
                f"Failed to insert goal '{task_group.plan_association.plan_title}' into database"
            )
    ...
    async def _save_single_task(
        ...
        task_data = {
            ...
            "metadata": {
                ...
                "assignee_names": [a.assignee_name for a in task.assignments]
                if task.assignments
                else [],
                "checklist_items": [
                    {"title": item.title} for item in task.details.checklist_items
                ]
                if task.details.checklist_items
                else [],
            },
            "created_at": datetime.utcnow().isoformat(),
            "updated_at": datetime.utcnow().isoformat(),
    ...
    async def update_task_planner_id(
        self, task_id: UUID, planner_task_id: str, planner_plan_id: Optional[str] = None
    ) -> None:
